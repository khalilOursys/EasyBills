...
export default function ListSaleInvoicePage() {
  ...
  const getAddRoute = () => {
    if (type === "SHIPPING_NOTE_INVOICE")
      return `/sale-invoice/add/SHIPPING_NOTE_INVOICE `
    else
  ...
  const handleEdit = (invoice: SaleInvoice) => {
    ...
    else if (invoice.type === "SALE_INVOICE")
      router.push(`/sale-invoice/update/SALE_INVOICE/${invoice.id}`);
    else if (invoice.type === "DELIVERY_NOTE")
      router.push(`/sale-invoice/update/DELIVERY_NOTE/${invoice.id}`);
    else
