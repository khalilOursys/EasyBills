...
export default function AddSaleInvoicePage() {
            ...
            if (client?.value) {
                ...
                try {
                    ...
                    const filteredInvoices = (data || []).filter(
                        (inv: any) => inv.client.id === client.value
                    );
                    setDeliveryNotes(filteredInvoices);
                } catch (error) {
                    showToast("Erreur lors du chargement des bons de livraison", "error");
                    setDeliveryNotes([]);
                } finally {
    ...
    const loadSelectedDeliveryNotes = async () => {
        ...
        try {
            ...
            // Combine all items from selected delivery notes
    ...
    const deliveryNoteOptions = deliveryNotes.map((note: any) => ({
        value: note.id,
    ...
    return (
        <Toast.Provider>
            <div className="p-6">
                <PageBreadcrumb pageTitle="Ajouter facture de vente" />
